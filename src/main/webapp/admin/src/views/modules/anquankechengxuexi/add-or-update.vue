<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="资源来源" prop="ziyuanlaiyuan">
          <el-input v-model="ruleForm.ziyuanlaiyuan" 
              placeholder="资源来源" clearable  :readonly="ro.ziyuanlaiyuan"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="资源来源" prop="ziyuanlaiyuan">
              <el-input v-model="ruleForm.ziyuanlaiyuan" 
                placeholder="资源来源" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.leibie = true;
          ...
          if(o=='ziyuanlaiyuan'){
            this.ruleForm.ziyuanlaiyuan = obj[o];
	    this.ro.ziyuanlaiyuan = true;
            continue;
          }
    ...
    info(id) {
      this.$http({
        url: `anquankechengxuexi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.kechengjieshao = this.ruleForm.kechengjieshao.replace(reg,'../../../ssm3s0eb/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `anquankechengxuexi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.anquankechengxuexiCrossAddOrUpdateFlag = false;
                  this.parent.search();
