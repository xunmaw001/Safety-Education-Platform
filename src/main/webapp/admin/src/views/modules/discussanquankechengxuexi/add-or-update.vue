...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `discussanquankechengxuexi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `discussanquankechengxuexi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.discussanquankechengxuexiCrossAddOrUpdateFlag = false;
                  this.parent.search();
